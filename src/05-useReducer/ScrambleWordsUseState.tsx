...
export const ScrambleWords = () => {
    ...
    const handleGuessSubmit = (e: React.FormEvent) => {
        ...
        if (guess != currentWord) {
            const newErrorValue = errorCounter + 1;
            setErrorCounter(newErrorValue)
            if (newErrorValue == 3) {
                setIsGameOver(true)
            }
        }
    ...
    const handlePlayAgain = () => {
        const newGameWords = shuffleArray(GAME_WORDS)
        setWords(shuffleArray(newGameWords))
        setCurrentWord(newGameWords[0])
        setScrambledWord(scrambleWord(newGameWords[0]))
        setPoints(0)
        setErrorCounter(0)
        setSkipCounter(0)
        setIsGameOver(false)
